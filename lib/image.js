...
exports.convert = function(imgBody, fileName, params, callback) {
  ...
  image.size(function(err, size) {
    ...
    // 元画像の比率と指定したパラメータの比率が同じ場合、リサイズのみ行う
    if (size.width * params.height === size.height * params.width) {
      image = resize(image, params.width, params.height);
    } else {
      ...
      if (resizeWidth || resizeHeight) {
        image = resize(image, resizeWidth, resizeHeight);
      }
